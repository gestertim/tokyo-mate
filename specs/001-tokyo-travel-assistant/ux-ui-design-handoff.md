...
分類是靜態 Tokyo Knowledge Base 瀏覽入口；切換分類直接顯示該分類完整項目，不呼叫 AI 或外部 API。AI 問東京的語意 Top 3–5 檢索與百科分類瀏覽是兩條不同流程。

百科卡片預設顯示標題與摘要，每張卡片可獨立展開／收合。展開後依資料存在性顯示亮點、建議方式、交通提示、必吃必買、建議停留時間、日夜差異、注意事項與實用日文；沒有資料的欄位不顯示空標題或空清單。

實用日文顯示日文原文與繁中意思，提供複製與播放語音。播放重用既有 AudioPlayer／TTS；離線或播放失敗時保留文字閱讀與複製，不宣稱離線語音可用。展開控制支援鍵盤並使用 `aria-expanded`／`aria-controls`，複製結果以可存取狀態訊息回饋。
