...
5. 開啟「東京百科」，依序切換「區域、交通、美食、購物、文化、緊急」六個分類。
   - **驗證點**：每個分類直接顯示該分類完整靜態項目；切換分類不發出 AI 或外部 API request，不以 AI Top 3–5 結果過濾百科內容。
6. 展開兩張百科卡片，再分別收合其中一張。
   - **驗證點**：每張卡片可獨立展開與收合；標題與摘要預設可見；有資料的亮點、建議方式、交通提示、必吃必買、建議停留時間、日夜差異、注意事項與實用日文正確顯示；缺少欄位不顯示空標題或空清單。
7. 在包含實用日文的卡片中檢查原文與繁中意思，使用鍵盤操作展開／收合並按下複製與播放語音。
   - **驗證點**：鍵盤可操作，展開控制具 `aria-expanded` 與 `aria-controls`；複製成功或失敗均有可存取狀態訊息；線上可播放語音。
8. 關閉網路並重新開啟東京百科，另模擬語音失敗。
   - **驗證點**：百科文字仍可閱讀，實用日文仍可複製；離線或語音失敗時不假裝播放成功，也不承諾離線語音。
...
### 4.4 東京百科卡片、六分類與 fallback 驗收
- 六分類必須各自呈現完整靜態資料，分類切換不得觸發 AI 或外部 API；AI 問東京仍使用 Top 3–5 語意檢索。
