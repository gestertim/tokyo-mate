...
5. **Given** 使用者進入東京百科並切換六個分類中的任一分類，**When** 分類切換完成，**Then** 系統直接顯示該靜態分類的完整知識項目，不呼叫 AI 或外部 API，且不以 AI Top 3–5 結果作為分類內容來源。
6. **Given** 使用者查看百科分類，**When** 卡片初次呈現或被展開，**Then** 卡片初次顯示標題與摘要，展開後依資料存在與否顯示詳細欄位；每張卡片可獨立展開與收合，不影響其他卡片。
7. **Given** 百科卡片包含實用日文，**When** 使用者展開卡片，**Then** 可閱讀日文原文與繁體中文意思、複製文字及播放語音；離線或語音失敗時仍可閱讀及複製，不將語音可用性誤報為成功。
...
- 當百科項目缺少部分 optional 欄位、複製失敗、語音播放失敗或裝置離線時，系統如何維持卡片可讀性、可操作性與可理解的 fallback？
...
- **FR-027**: 東京百科必須提供「區域、交通、美食、購物、文化、緊急」六個靜態分類；切換分類時必須直接呈現該分類完整的 Tokyo Knowledge Base 項目，不得先取得 AI Top 3–5 檢索結果再過濾。分類切換不得呼叫 AI 或外部 API；AI 問東京仍維持原有依語意選取 Top 3–5 筆相關知識的契約。
- **FR-028**: 東京百科項目預設必須顯示標題與摘要；每張卡片都必須可獨立展開與收合。展開時，僅在資料存在時顯示亮點、建議方式、交通提示、必吃必買、建議停留時間、日夜差異、注意事項與實用日文；缺少的 optional 欄位不得顯示空標題或空清單，且一張卡片的展開狀態不得影響其他卡片。
- **FR-029**: 東京百科的實用日文必須顯示日文原文與台灣繁體中文意思，提供複製與播放語音功能，並重用既有 AudioPlayer 與 TTS 能力。百科文字必須維持離線可讀；離線或語音失敗時仍可閱讀及複製文字，但不得承諾離線語音播放。
- **FR-030**: 東京百科的展開／收合控制必須支援鍵盤操作，使用 `aria-expanded` 與 `aria-controls`；複製成功或失敗必須提供可存取狀態訊息。百科畫面必須維持 FR-025／SC-014 viewport matrix，且不得產生水平捲動、控制項重疊或內容裁切。
...
- **SC-015**: 東京百科六分類驗收中，每一分類切換均能顯示該分類完整靜態項目；分類切換期間不產生 AI 或外部 API request，且 AI 問東京的 Top 3–5 語意檢索仍維持原契約。
- **SC-016**: 百科卡片驗收中，每張卡片均可獨立展開與收合；標題與摘要預設可見，所有已提供詳細欄位正確呈現，缺少欄位不產生空標題或空清單，且一張卡片的狀態不改變其他卡片。
- **SC-017**: 百科實用日文、離線與無障礙驗收中，日文原文／繁中意思可閱讀並可複製，線上語音可播放；離線或語音失敗仍可閱讀及複製。鍵盤、`aria-expanded`、`aria-controls`、複製狀態訊息與 FR-025／SC-014 viewport matrix 均通過，且無水平捲動、重疊或裁切。
